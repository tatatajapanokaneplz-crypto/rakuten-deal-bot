...
【2026-07 修正(本文・返信の単調さ)】本文の切り方、返信の語尾・締めの句読点が
それぞれ偏っていた問題を修正し、例のパターンを大幅に拡充した。

【2026-07 初の実投稿確認】--dry-run なしで実行し、実際にX/Threadsへの投稿が
成功することを確認した。

【2026-07 修正(ジャンルに合わない語彙の問題)】実際の投稿を見たところ、
オムツ(マミーポコパンツ)の返信文で「このスペックで5,956円はコスパ凄すぎて」
のように、ガジェット系商品を前提にした語彙(スペック等)が使われてしまって
いた。原因は _generate_reply_hook が商品名・ジャンルを一切受け取っておらず、
ペルソナ設定(トレンドの商品やガジェットを楽しむ)に引っ張られて、常に
ガジェット寄りの語彙を選んでしまっていたため。
そのため、_generate_teaser・_generate_reply_hook の両方に item.item_name を
「語彙選びの参考情報」として渡すよう修正した。ただし、本文・返信文の中に
実際の商品名を書いてしまうと本来の設計(リンクプレビューとの重複回避)が
崩れるため、「この商品名は参考情報であり、文章には一切書かないこと」と
明示的に指示している。
"""
...
def _generate_teaser(item: RakutenItem, track: str) -> str:
    ...
    商品名は「語彙選びの参考」としてのみ渡し、本文には書かせない
    (ガジェット向けの語彙を食品・日用品などに誤用しないようにするため)。
    """
    if track == "timesale":
    ...
    prompt = (
        ...
        f"【参考情報(文章には絶対に書かないこと)】実際の商品: {item.item_name}\n"
        f"この情報は、商品ジャンルに合った自然な語彙を選ぶためだけに使ってください"
        f"(例: ガジェットなら「スペック」、食品なら「美味しさ」、日用品なら「使い勝手」"
        f"のように、ジャンルにふさわしい言葉を選ぶ)。商品名や具体的な特徴そのものは"
        f"文章に書かないでください。\n\n"
        f"絶対に守ること:\n"
...
def _generate_reply_hook(item: RakutenItem) -> str:
    ...
    商品名は「語彙選びの参考」としてのみ渡し、返信文には書かせない
    (ガジェット向けの語彙を食品・日用品などに誤用しないようにするため)。
    """
    ...
    prompt = (
        ...
        f"【参考情報(文章には絶対に書かないこと)】実際の商品: {item.item_name}\n"
        f"この情報は、商品ジャンルに合った自然な語彙を選ぶためだけに使ってください"
        f"(例: ガジェットなら「スペック」、食品なら「美味しさ」、日用品や消耗品なら"
        f"「使い勝手」「コスパ」のように、ジャンルにふさわしい言葉を選ぶ。ガジェット"
        f"以外の商品に「スペック」のような技術用語を使うのはNG)。商品名や具体的な"
        f"特徴そのものは文章に書かないでください。\n\n"
